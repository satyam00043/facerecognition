...
class Login_Window:
    def __init__(self, root):
        ...
        get_str.place(x=150, y=80)

        User_str = Label(frame, text="User_Name", font=("times new roman", 20, "bold"), fg="white", bg="black")
        User_str.place(x=90, y=125)
        ...
        txtuser.place(x=90, y=160, width=300)

        password = Label(frame, text="Password", font=("times new roman", 20, "bold"), fg="white", bg="black")
        password.place(x=90, y=195)

        txtpass = ttk.Entry(frame, textvariable=self.txtpass, font=("times new roman", 15, "bold"), show="*")
        txtpass.place(x=90, y=230, width=300)

        btn_login = Button(frame, text="Login", font=("times new roman", 20, "bold"),fg="green", command=self.login)
        btn_login.place(x=90, y=300, width=100)

        btn_register = Button(frame, text="Register", font=("times new roman", 25, "bold"), command=self.open_register_window)
        btn_register.place(x=250, y=300, width=150,height=50 )

        btn_forgot = Button(frame, text="Forgot Password?", font=("times new roman", 15, "bold"),fg="red", command=self.open_forgot_password_window)
        btn_forgot.place(x=150, y=360, width=150)
...
def insert_user(username, password):
    ...
    try:
        ...
        cursor.execute(
            "INSERT INTO users (username, password, profile) VALUES (%s, %s, %s)",
            (username, hashed_password.decode('utf-8'), 'default profile')
        )
        connection.commit()
